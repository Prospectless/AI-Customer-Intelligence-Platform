    ...
    with tab2:
        ...
        with col_l:
            if os.path.exists(dendrogram_path):
                st.image(dendrogram_path, caption="Dendrogram (Ward, 5K Örneklem)", use_container_width=True)
        with col_r:
            if os.path.exists(hc_sil_path):
                st.image(hc_sil_path, caption="HC Silhouette Scores", use_container_width=True)
    ...
    with tab1:
        ...
        with col_r:
            if os.path.exists(cm_path):
                st.image(cm_path, caption="Confusion Matrix (XGBoost)", use_container_width=True)
