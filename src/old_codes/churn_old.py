...
plt.colorbar()
plt.xticks([0, 1], ["Satın Almaz", "Satın Alır"])
plt.yticks([0, 1], ["Satın Almaz", "Satın Alır"])
...
df_result = df.copy()
df_result["churn_prob_lr"] = lr.predict_proba(X_test_scaled if hasattr(lr, 'predict_proba') else X_test)[:, 1] if False else np.nan
df_result = df.copy()
...
#############################################
# 15. SEGMENT BAZLI CHURN ANALİZİ
#############################################

print("\n" + "=" * 60)
print("SEGMENT BAZLI CHURN ANALİZİ")
print("=" * 60)

segment_churn = predictions.groupby("RFM_Segment").agg(
    Musteri_Sayisi=("CustomerID", "count"),
    Gercek_Churn_Orani=("Gercek_PurchaseStatus", "mean"),
    XGB_Tahmin_Orani=("XGB_Tahmin", "mean"),
    Ort_Churn_Olasiligi=("XGB_Olasilik", "mean")
