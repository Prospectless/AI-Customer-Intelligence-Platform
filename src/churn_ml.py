...
# ✅ FIX: Tümel dataset'i de scale et
...
#############################################
# 14. ✅ TÜMEL DATASET PREDICTIONS (FIX - 491K)
#############################################

print("\n" + "=" * 60)
print("TÜMEL DATASET PREDICTIONS (491K MÜŞTERİ)")
print("=" * 60)

# ✅ FIX: Logistic Regression (best model) ile tümel dataset
y_prob_full_lr = lr.predict_proba(X_full_scaled)[:, 1]

# ✅ FIX: CHURN probability = 1 - PURCHASE probability
# PurchaseStatus=1 means satın aldı (purchased)
# PurchaseStatus=0 means satın almadı (churned)
churn_prob_full = 1 - y_prob_full_lr

print(f"Tümel predictions: {len(churn_prob_full)}")
print(f"Churn probability range: {churn_prob_full.min():.4f} - {churn_prob_full.max():.4f}")
print(f"Ortalama churn riski: {churn_prob_full.mean():.4f}")

# ✅ Save full predictions
predictions_full = pd.DataFrame({
    "CustomerID": df["CustomerID"],
    "RFM_Segment": df["RFM_Segment"],
    "PurchaseStatus": df["PurchaseStatus"],
    "Gercek_Churn": 1 - df["PurchaseStatus"],
    "LR_Churn_Probability": churn_prob_full.round(4),
})

predictions_full.to_csv(f"{OUTPUT_DIR}/churn_predictions.csv", index=False)
print(f"\n✅ Saved: {OUTPUT_DIR}/churn_predictions.csv ({len(predictions_full)} rows)")

#############################################
# 15. ✅ SEGMENT BAZLI CHURN ANALİZİ (FIX - 491K)
#############################################

print("\n" + "=" * 60)
print("SEGMENT BAZLI CHURN ANALİZİ (FIXED - 491K)")
print("=" * 60)

# ✅ FIX: Tümel dataset'ten segment churn hesapla
segment_churn = predictions_full.groupby("RFM_Segment").agg(
    Musteri_Sayisi=("CustomerID", "count"),
    Gercek_Churn_Orani=("Gercek_Churn", "mean"),  # Actual churn (0-1)
    Tahmin_Churn_Orani=("LR_Churn_Probability", "mean"),  # ML predicted churn (0-1)
    Ort_Churn_Olasiligi=("LR_Churn_Probability", "mean")  # Average churn probability
).round(3).sort_values("Gercek_Churn_Orani", ascending=False)

# Convert to percentage for display
segment_churn_display = segment_churn.copy()
segment_churn_display["Gercek_Churn_Orani"] = (segment_churn_display["Gercek_Churn_Orani"] * 100).round(1)
segment_churn_display["Tahmin_Churn_Orani"] = (segment_churn_display["Tahmin_Churn_Orani"] * 100).round(1)

print(segment_churn_display)
segment_churn.to_csv(f"{OUTPUT_DIR}/segment_churn_analysis.csv")

print(f"\n✅ Saved: {OUTPUT_DIR}/segment_churn_analysis.csv")
...
print(f"\nModel Sonuçları (Test Set):")
print(model_comparison.to_string())
print(f"\nEn iyi model: {best_model_name}")

print(f"\nSegment Churn Analizi (Tümel Dataset - 491K):")
print(segment_churn_display.to_string())

print(f"\n📊 Kaydedilen dosyalar:")
for f_name in sorted(os.listdir(OUTPUT_DIR)):
    file_path = os.path.join(OUTPUT_DIR, f_name)
    file_size = os.path.getsize(file_path) / 1024  # KB
    print(f"  ✅ {OUTPUT_DIR}/{f_name} ({file_size:.1f} KB)")

print("\n" + "=" * 60)
print("✅ ML-BASED CHURN PREDICTIONS READY (491K)")
print("=" * 60)